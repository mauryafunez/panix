...
void DrawInterruptIndicator(tty_vga_color color)
{
    graphics::Framebuffer* fb = graphics::getFramebuffer();
    uint32_t cornerX = (fb->getWidth() - 1) - INDICATOR_WIDTH;
    graphics::putrect(cornerX, 0, INDICATOR_WIDTH, INDICATOR_HEIGHT, color);
}
